import { type VariantProps } from "tailwind-variants";
export declare const buttonToggleGroup: import("tailwind-variants").TVReturnType<{
    roundedSize: {
        sm: string;
        md: string;
        lg: string;
        xl: string;
        full: string;
    };
}, undefined, "inline-flex border border-gray-300 overflow-hidden", {
    roundedSize: {
        sm: string;
        md: string;
        lg: string;
        xl: string;
        full: string;
    };
}, undefined, import("tailwind-variants").TVReturnType<{
    roundedSize: {
        sm: string;
        md: string;
        lg: string;
        xl: string;
        full: string;
    };
}, undefined, "inline-flex border border-gray-300 overflow-hidden", unknown, unknown, undefined>>;
export type ButtonToggleGroupTheme = string;
export declare const buttonToggle: {
    button: import("tailwind-variants").TVReturnType<{
        selected: {
            true: string;
            false: string;
        };
        size: {
            sm: string;
            md: string;
            lg: string;
            xl: string;
        };
        roundedSize: {
            sm: string;
            md: string;
            lg: string;
            xl: string;
            full: string;
        };
        color: {
            primary: string;
            secondary: string;
            gray: string;
            red: string;
            orange: string;
            amber: string;
            yellow: string;
            lime: string;
            green: string;
            emerald: string;
            teal: string;
            cyan: string;
            sky: string;
            blue: string;
            indigo: string;
            violet: string;
            purple: string;
            fuchsia: string;
            pink: string;
            rose: string;
            none: string;
        };
    }, undefined, "relative flex items-center transition-all duration-200  focus:outline-none border-r last:border-r-0 dark:bg-white dark:text-gray-800", {
        selected: {
            true: string;
            false: string;
        };
        size: {
            sm: string;
            md: string;
            lg: string;
            xl: string;
        };
        roundedSize: {
            sm: string;
            md: string;
            lg: string;
            xl: string;
            full: string;
        };
        color: {
            primary: string;
            secondary: string;
            gray: string;
            red: string;
            orange: string;
            amber: string;
            yellow: string;
            lime: string;
            green: string;
            emerald: string;
            teal: string;
            cyan: string;
            sky: string;
            blue: string;
            indigo: string;
            violet: string;
            purple: string;
            fuchsia: string;
            pink: string;
            rose: string;
            none: string;
        };
    }, undefined, import("tailwind-variants").TVReturnType<{
        selected: {
            true: string;
            false: string;
        };
        size: {
            sm: string;
            md: string;
            lg: string;
            xl: string;
        };
        roundedSize: {
            sm: string;
            md: string;
            lg: string;
            xl: string;
            full: string;
        };
        color: {
            primary: string;
            secondary: string;
            gray: string;
            red: string;
            orange: string;
            amber: string;
            yellow: string;
            lime: string;
            green: string;
            emerald: string;
            teal: string;
            cyan: string;
            sky: string;
            blue: string;
            indigo: string;
            violet: string;
            purple: string;
            fuchsia: string;
            pink: string;
            rose: string;
            none: string;
        };
    }, undefined, "relative flex items-center transition-all duration-200  focus:outline-none border-r last:border-r-0 dark:bg-white dark:text-gray-800", unknown, unknown, undefined>>;
    content: import("tailwind-variants").TVReturnType<{} | {} | {}, undefined, "flex items-center w-full overflow-hidden relative", {} | {}, undefined, import("tailwind-variants").TVReturnType<unknown, undefined, "flex items-center w-full overflow-hidden relative", unknown, unknown, undefined>>;
    text: import("tailwind-variants").TVReturnType<{
        selected: {
            true: string;
            false: string;
        };
    }, undefined, "transition-all duration-200 ml-0", {
        selected: {
            true: string;
            false: string;
        };
    }, undefined, import("tailwind-variants").TVReturnType<{
        selected: {
            true: string;
            false: string;
        };
    }, undefined, "transition-all duration-200 ml-0", unknown, unknown, undefined>>;
};
export type ButtonToggleTheme = {
    button?: string;
    content?: string;
    text?: string;
};
export type ButtonToggleVariants = VariantProps<typeof buttonToggle.button>;
export type ButtonToggleContentVariants = VariantProps<typeof buttonToggle.content>;
export type ButtonToggleTextVariants = VariantProps<typeof buttonToggle.text>;
