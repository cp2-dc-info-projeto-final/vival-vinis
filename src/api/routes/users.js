...
router.get('/', verifyToken, isAdmin, async function(req, res) {
  try {
    const result = await pool.query('SELECT id, login, email, senha, role FROM usuario ORDER BY id');
    res.json({
      success: true,
      data: result.rows
    });
...
router.get('/nome/:nome', async function(req, res, next) {
  try {
    ...
    const result = await pool.query('SELECT * FROM usuario WHERE login LIKE $1', ['%'+nome+'%']);
